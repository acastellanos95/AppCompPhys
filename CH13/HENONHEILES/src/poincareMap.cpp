...
int main (int argc, char * * argv) {
  ...
  input.declare("E",       "Energy of the system", 0.06);
  input.declare("Y",       "Y position [suggest <0.4]", 0.2); 
  input.declare("PY",       "Y momentum [suggest <0.4]", 0.2); 
  input.declare("lambda",   "lambda [suggest 1 or zero] ", 1.0); 
  input.declare("alpha",   "alpha [order one] ", 1.0); 
  try {
  ...
  double alpha=input.getByName("alpha");
  ...
  //
  double lambda=input.getByName("lambda");
  Genfun::GENFUNCTION H = 
    0.5*(p[0]*p[0]+p[1]*p[1])+1/2.0*(alpha*r[0]*r[0]+r[1]*r[1])+ lambda*(r[0]*r[0]*r[1]-1/3.0*r[1]*r[1]*r[1]);
  //
